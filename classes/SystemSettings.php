		...
		if (isset($_FILES['banners']) && count($_FILES['banners']['tmp_name']) > 0) {
			...
			foreach ($_FILES['banners']['tmp_name'] as $k => $v) {
				if (!empty($_FILES['banners']['tmp_name'][$k])) {
					$ext = pathinfo($_FILES['banners']['name'][$k], PATHINFO_EXTENSION);
					$fname = $banner_path . uniqid() . "_$k.$ext";
					$upload_path = base_app . $fname;
					if (move_uploaded_file($_FILES['banners']['tmp_name'][$k], $upload_path)) {
