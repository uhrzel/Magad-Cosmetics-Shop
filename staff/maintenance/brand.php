		...
		<div class="container-fluid">
			<table class="table table-hover table-striped" id="list">
				...
				<tbody>
					...
					// Fetch user_id_brand from session
					$current_user_id = isset($_SESSION['userdata']['id']) ? $_SESSION['userdata']['id'] : null;

					// Update the query to filter by user_id_brand
					$qry = $conn->query("SELECT * FROM `brands` WHERE `delete_flag` = 0 AND `user_id_brand` = {$current_user_id} ORDER BY `name` ASC");
